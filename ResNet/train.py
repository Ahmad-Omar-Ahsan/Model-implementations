...
def pipeline():
    ...
    """
    conf = Config()
    net=ResNet101(conf.img_channels, num_classes=conf.num_classes).to(conf.device)
    train_loader, val_loader, test_loader = load_dataset(conf.batch_size)
    loss_fn = LabelSmoothingLoss(conf.num_classes)
    optimizer = torch.optim.AdamW(net.parameters(), lr=conf.lr)
    wandb.login(key=conf.wandb_key)
    wandb.init(project=conf.project,config={'batch_size': conf.batch_size,"optimizer": "AdamW", "loss_fn":"crossentropy with labelsmoothing","lr":conf.lr})
